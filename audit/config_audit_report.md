...
コミット(このブランチ上、各 1 コミット = 1 MR 相当。`audit/patches/` に format-patch 出力):

| finding | commit | patch |
|---|---|---|
| A1 | `c29878c` du_high: wire f4_enable_occ into pucch builder | `0001-du_high-wire-f4_enable_occ-into-pucch-builder.patch` |
| B1 | `63769be` sched: derive max mimo layers from builder params | `0002-sched-derive-max-mimo-layers-from-builder-params.patch` |
| A5 | `8adc6a0` du_high: remove unused autostart metrics field | `0003-du_high-remove-unused-autostart-metrics-field.patch` |

検証(`build_audit/`、`cmake -GNinja -DBUILD_TESTING=ON -DCMAKE_BUILD_TYPE=Release`、3 修正適用後のツリー):

- ビルド: `sched_config_test`・`serving_cell_config_converter_test`・`ocudu_du_high_unit_helpers` 全て成功(442 targets)。
- `sched_config_test`: **36 tests / 4 suites PASSED**(B1 の新規 positive テスト
  `serving_cell_config_factory_test` 2 ケースを含む。nof_ant=2 → maxMIMO-Layers=2、
  明示 max_nof_layers=1 → 1 を確認)。
- `serving_cell_config_converter_test`: **23 tests PASSED**(既存の `max_mimo_layers_present`
  アサーションは値非依存のため回帰なし)。
- `clang-format --dry-run -Werror`: 変更 5 ファイル全てクリーン。
- A1 は translator の単体テスト基盤が存在しないため専用テストなし(F1 の既存実装のミラー、
  1 行のデータ配線)。コンパイル＋上記スイートで回帰なしを確認。

補足: 検証環境には gtest/fftw3/yaml-cpp/sctp/mbedtls を追加インストールした
(コンテナ初期状態には無し)。
