...
- **確定(コード上で立証済み)の不具合: 12 件**(うち XS: 8 件、S: 4 件)+ 表記/コメントの
  軽微な不整合 5 件。**仕様テーブルの「値の転記ミス」は 1 件も検出されなかった**
  (MCS/CQI/β オフセット/DMRS 位置/デフォルト TD 割当/アンテナポート/PRACH NCS/
  PUCCH Table 9.2.1-1 は行単位で全一致)。問題は主に **(a) 検証関数の抜け・型・制御フロー、
  (b) 内部設定→ASN.1 変換の論理バグ** に集中している。
...
- 未監査/部分監査: `lib/ran/band_helper.cpp`(帯域表)、`pucch_info.cpp` の全数式導出
  ...
  NTN(sib19)変換、`asn1_ntn_config_helpers.cpp`。必要なら次フェーズで対象化。
